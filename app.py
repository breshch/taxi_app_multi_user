# app.py — ПОЛНАЯ ВЕРСИЯ с фиксом Google OAuth + фикс сессии
import os
import json
import base64
import shutil
...
def load_session_from_disk():
    """Возвращает dict с username и session_start если сессия валидна, иначе None"""
    try:
...
def verify_password(password: str, hash: str) -> bool:
    try:
        return bcrypt.verify(password.strip().encode('utf-8')[:72], hash)
    except:
        return False
...
def show_main_page():
    ...
    if not open_shift_data:
        ...
        with st.expander("💸 Дополнительные расходы", expanded=False):
            ...
            with col1:
                exp_desc = st.selectbox("📝 Тип расхода", POPULAR_EXPENSES, key="exp_desc", width='stretch')
            with col2:
        ...
        with st.expander("⛽ Закрыть смену", expanded=False):
            ...
            c1, c2 = st.columns(2)
            with c1:
                cons = st.number_input("⛽ Расход (л/100км)", value=float(last_cons), step=0.5, key="cons_close", width='stretch')
            with c2:
                price = st.number_input("💰 Цена топлива (₽/л)", value=float(last_price), step=1.0, key="fuel_close", width='stretch')
...
if __name__ == "__main__":
    ...
    # ✅ ИСПРАВЛЕНИЕ СЕССИИ:
    # Раньше init_session() шёл ПЕРВЫМ и при перезапуске Streamlit (закрытие браузера)
    # видел что session_start отсутствует → создавал новый session_start и сохранял
    # пустой файл (без username), затирая старую сессию. После этого load_session_from_disk
    # уже читал файл без username и не восстанавливал вход.
    #
    # Правильный порядок: СНАЧАЛА читаем файл и восстанавливаем session_start + username,
    # ПОТОМ вызываем init_session() — он видит что session_start уже есть и не трогает файл.
    if "username" not in st.session_state:
        saved_data = load_session_from_disk()
        if saved_data:
            username = saved_data.get("username")
            start_str = saved_data.get("session_start")
            if username and start_str:
                try:
                    st.session_state.session_start = datetime.fromisoformat(start_str).replace(tzinfo=MOSCOW_TZ)
    ...
    with st.sidebar:
        ...
        st.divider()
        if st.button("🏠 Главная страница", width='stretch'):
            st.session_state.page = "main"
            st.rerun()
        if st.button("📊 Отчёты", width='stretch'):
            st.session_state.page = "reports"
            st.rerun()
        if st.button("🔧 Админка", width='stretch'):
