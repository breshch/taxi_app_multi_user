                ...
                with st.container():
                    ...
                    with cols[3]:
                        ...
                        if st.session_state.confirm_delete.get(confirm_key, False):
                            st.markdown('<div class="warning-box">Подтвердите удаление</div>', unsafe_allow_html=True)
...
# Информация в сайдбаре с навигацией (РУССКИЕ НАЗВАНИЯ)
with st.sidebar:
...
if st.session_state["page"] == "main":
    ...
    try:
        ...
        # Вызываем основную функцию отчётов (предполагаем, что она есть)
    ...
    try:
        ...
        # Вызываем основную функцию админки (предполагаем, что она есть)
