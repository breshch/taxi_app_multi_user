    ...
    if not open_shift_data:
        ...
        if orders:
            ...
            for i, (_, tp, am, ti, _, tot, _, tm) in enumerate(orders, 1):
                ...
                cols[0].markdown(
                    f"**#{i}** {tm or ''} {'💵' if tp=='нал' else '💳'} {am:.0f}₽"
                )
            ...
            st.metric(
                "ДОХОД",
                f"{totals.get('нал',0)+totals.get('карта',0)+totals.get('чаевые',0):.0f}₽",
            )
        ...
        with st.expander("💸 ДОП ЗАТРАТЫ", expanded=False):
            ...
            with c1:
                exp_desc = st.selectbox(
                    "Что", options=[""] + POPULAR_EXPENSES, key="exp_desc"
                )
            with c2:
                exp_amt = st.number_input(
                    "Сумма", min_value=0.0, step=50.0, value=100.0, key="exp_amt"
                )
            with c3:
        ...
        total_income = (
            totals.get("нал", 0) + totals.get("карта", 0) + totals.get("чаевые", 0)
        )
        ...
        col3.metric(
            "📈 Чистыми",
            f"{total_income - total_extra:.0f}₽",
            delta=f"{total_income - total_extra:.0f}₽",
        )
        ...
        with st.expander("🔒 ЗАКРЫТЬ СМЕНУ", expanded=False):
            ...
            if st.button(
                "🔒 Закрыть смену", width="stretch", type="primary", key="close_shift"
            ):
...
def show_reports_page():
    ...
    try:
        ...
        ym = st.selectbox(
            "📅 Выберите месяц", year_months, format_func=format_month_option, index=0
        )
        ...
        c3.metric(
            "📈 Прибыль",
            f"{profit:.0f} ₽",
            delta=f"{stats.get('рентабельность', 0):.1f}%" if profit > 0 else None,
        )
...
with st.sidebar:
    ...
    if st.button(
        "📋 Главная",
        width="stretch",
        type="primary" if st.session_state.get("page") == "main" else "secondary",
    ):
        st.session_state.page = "main"
        st.rerun()
    ...
    if st.button(
        "📊 Отчёты",
        width="stretch",
        type="primary" if st.session_state.get("page") == "reports" else "secondary",
    ):
