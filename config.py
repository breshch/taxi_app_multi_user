...
POPULAR_EXPENSES = ["🚗 Мойка","💧 Омывайка","🍔 Еда","☕ Кофе","🚬 Сигареты","🔧 Мелкий ремонт","🅿️ Парковка","💰 Штраф","🧴 Очиститель","🔋 Зарядка","🧰 Инструмент","📱 Связь","🚕 Аренда","💊 Аптека"
]
...
def get_db_schema():
    return ["""CREATE TABLE IF NOT EXISTS shifts (
            ...
            is_open INTEGER DEFAULT 1, opened_at TEXT, closed_at TEXT)""","""CREATE TABLE IF NOT EXISTS orders (
            ...
            beznal_added REAL DEFAULT 0, order_time TEXT)""","""CREATE TABLE IF NOT EXISTS extra_expenses (
            id INTEGER PRIMARY KEY AUTOINCREMENT, shift_id INTEGER,
            amount REAL DEFAULT 0, description TEXT, created_at TEXT)""","""CREATE TABLE IF NOT EXISTS accumulated_beznal (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def check_and_create_tables(db_path: str):
    try:
        ...
        if not cur.fetchone():
            cur.execute("INSERT INTO accumulated_beznal (driver_id, total_amount, last_updated) VALUES (1, 0, ?)",
