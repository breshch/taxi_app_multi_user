# pages_imports.py — ПОЛНОСТЬЮ ЗАМЕНИТЕ СОДЕРЖИМОЕ
"""
Отчёты и вспомогательные функции для taxi app.
Использует функции БД из app.py, не дублирует логику.
"""
...
# Импортируем только необходимые функции из app.py
from app import (
    get_db_connection,
...
def ensure_report_indexes():
    """Создаёт индексы для быстрых отчётов."""
...
def get_available_year_months_cached() -> List[str]:
    """Список доступных периодов YYYY-MM."""
    ...
    c.execute("""
        ...
        WHERE date IS NOT NULL 
          ...
          AND is_open = 0
          AND EXISTS (
              SELECT 1 FROM orders o WHERE o.shift_id = shifts.id
          )
        ORDER BY 1 DESC
    """)
    rows = c.fetchall()
...
def get_month_totals_cached(year_month: str) -> Dict:
    """Итоги по месяцам: нал/карта/чаевые/безнал."""
    ensure_report_indexes()
    conn = get_db_connection()
    c = conn.cursor()
    c.execute(
...
def get_month_statistics(year_month: str) -> Dict:
    """Полная статистика месяца."""
    ensure_report_indexes()
    conn = get_db_connection()
    c = conn.cursor()
    c.execute(
        "SELECT COUNT(*) FROM shifts WHERE date LIKE ? AND is_open = 0",
        (f"{year_month}%",),
    )
    ...
    c.execute("""
        ...
        WHERE s.date LIKE ? AND s.is_open = 0
    """, (f"{year_month}%",))
    ...
    c.execute(
        "SELECT SUM(fuel_liters * fuel_price) FROM shifts WHERE date LIKE ? AND is_open = 0",
        (f"{year_month}%",),
    )
...
def format_month_option(s: Optional[str]) -> str:
    """Форматирует YYYY-MM → '2024-03 (март)'."""
    if not s:
        return "—"
    s_str = str(s)
    if len(s_str) < 7:
...
def recalc_full_db() -> float:
    """Пересчитывает все комиссии/безнал по текущим ставкам."""
    conn = get_db_connection()
    c = conn.cursor()
...
def reset_db():
    """Удаляет текущую БД пользователя."""
    db_path = get_current_db_name()
    if os.path.exists(db_path):
        os.remove(db_path)

def normalize_shift_dates() -> tuple[int, int]:
    """Нормализует даты смен в формате YYYY-MM-DD."""
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT id, date FROM shifts")
    rows = c.fetchall()
    fixed = skipped = 0
    for shift_id, date_str in rows:
        try:
            ...
            new_val = None
        old_clean = date_str.strip()
        if new_val and new_val != old_clean:
            c.execute("UPDATE shifts SET date = ? WHERE id = ?", (new_val, shift_id))
            fixed += 1
        else:
            skipped += 1
    conn.commit()
...
# ЭКСПОРТ ОТЧЁТОВ
# ==============================
def export_month_to_excel(year_month: str) -> bytes:
    """Экспорт отчёта месяца в Excel."""
    totals = get_month_totals_cached(year_month)
    stats = get_month_statistics(year_month)
    df_shifts = get_month_shifts_details_cached(year_month)
    with pd.ExcelWriter("отчет.xlsx", engine="openpyxl") as writer:
        summary = pd.DataFrame([totals | stats]).T
        summary.columns = ["Значение"]
